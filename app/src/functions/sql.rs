...
impl PgsqlService {
    ...
    pub async fn get_random_domain(
        &self,
        table_name: &str,
        columns: &[&str],
        ...
        for (i, (key, value)) in conditions.iter().enumerate() {
            ...
            if let Some(stripped_value) = value.strip_prefix('!') {
                ...
                params.push(stripped_value.to_string()); // 移除 '!' 后存入参数
            } else {
                // 否则默认是 = 条件
                where_clause.push_str(&format!(" AND {} = ${}", key, i + 1));
    ...
    pub async fn update_data(
        ...
        // 校验字段名合法性
        let all_keys = data.keys().chain(conditions.keys());
        if !all_keys.clone().all(|k| {
            k.chars()
                .all(|c| c.is_alphanumeric() || c == '_' || c == '-')
        }) {
            return Err(StatusCode::BAD_REQUEST);
        ...
        // 拒绝空数据或空条件
        if data.is_empty() || conditions.is_empty() {
            return Err(StatusCode::BAD_REQUEST);
        }
        ...
        // 构建WHERE子句（保持参数顺序）
        let mut sorted_conds: Vec<_> = conditions.iter().collect();
        sorted_conds.sort_by_key(|(k, _)| *k);
        let where_clause = sorted_conds
            .iter()
            .enumerate()
            .map(|(i, (k, _))| {
        ...
        // 绑定WHERE参数（按字符串处理）
        // for (_, value) in sorted_conds {
        //     query = query.bind(*value);
        // }
        // 绑定 WHERE 参数
        for (key, value) in sorted_conds {
            if key.to_lowercase() == "id" {
                // 特别处理 WHERE 中的 ID
                let id = value.parse::<i32>().map_err(|_| {
                    eprintln!("Invalid ID format in WHERE: {}", value);
