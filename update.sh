...
new="/www/MirrorElf_New/app/config/IP2LOCATION-LITE-DB3.BIN"
...
# 检查配置文件是否包含必要标记
if grep -q "^SEOFunctions:" "$config_file" && grep -q "^AccessPolicy:" "$config_file" && ! grep -q "friend_links" "$config_file"; then
  # 替换内容
  sed -i.bak '
